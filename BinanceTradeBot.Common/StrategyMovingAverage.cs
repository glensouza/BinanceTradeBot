...
namespace BinanceTradeBot.Common
{
    ...
    public class StrategyMovingAverage : IStrategies
    {
            ...
            bool testMode = true)
        {
            ...
            ApiClient apiClient = new ApiClient(binanceApiKey, binanceApiSecret);
            this.binanceClient = new BinanceClient(apiClient);

            //Retrieve historical data
            IEnumerable<Candlestick> historicalData =
                this.binanceClient.GetCandleSticks(symbol, TimeInterval.Days_1, limit: longTermMA).Result;
        ...
        public async Task RunStrategyAsync()
        {
            switch (this.strategy)
            {
                case MovingAverageStrategies.SimpleMovingAverage:
                    ...
                    {
                        //Buy BTC
                        dynamic order = await this.binanceClient.PostNewOrderTest("BTCUSDT", 0.001m, 0m, OrderSide.BUY, OrderType.MARKET);
    ...
    */
    ...
    class Strategy {
        ...
        public Strategy() {
            ...
            trainedModel = mlContext.Model.Load("path to your model file", out var modelSchema);
        }
        ...
        var predictionEngine = mlContext.Model.CreatePredictionEngine<MarketData, MarketPrediction>(trainedModel);
        var marketData = new MarketData() { 
            // Insert the features you want to use for prediction here, such as currentPrice, wma20, wma40
        };
        var prediction = predictionEngine.Predict(marketData);

        // Check if the model predicts that the price will rise and we're not in a position
        if (prediction.PredictedLabel && !inPosition) {
            buyTime = DateTime.Now;
            buyPrice = currentPrice;
            stopLoss = buyPrice * 0.95m; //stop loss at 5% below the buy price
            inPosition = true;
    ...
    public class MarketData {
        // properties for your features
    }

    // Define the output data for the model
